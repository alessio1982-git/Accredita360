...
async function checkStruttura() {
  const selectResp = await fetch(`${SUPABASE_URL}/rest/v1/users?email=eq.struttura@demo.it`, {
    headers: {
      'apikey': SUPABASE_KEY,
  ...
  console.log("struttura@demo.it Row:", data);
}

checkStruttura().catch(console.error);
